...
public final class Timer {
    ...
    public long getRemainingTime() {
        if (mState == State.RUNNING || mState == State.EXPIRED) {
    ...
    Timer start() {
        ...
        return new Timer(mId, State.RUNNING, mLength, mTotalLength, now(), mRemainingTime, mLabel,
                mDeleteAfterUse);
    }
    ...
    Timer pause() {
        if (mState == State.PAUSED || mState == State.RESET) {
            return this;
        } else if (mState == State.EXPIRED) {
            return reset();
        }

        final long remainingTime = getRemainingTime();
        return new Timer(mId, State.PAUSED, mLength, mTotalLength, Long.MIN_VALUE, remainingTime, mLabel,
                mDeleteAfterUse);
    }
     ...
     */
    Timer expire() {
        if (mState == State.EXPIRED || mState == State.RESET) {
            return this;
        }

        return new Timer(mId, State.EXPIRED, mLength, mTotalLength, mLastStartTime, mRemainingTime,
    ...
    Timer reset() {
        ...
        return new Timer(mId, State.RESET, mLength, mLength, Long.MIN_VALUE, mLength, mLabel,
                mDeleteAfterUse);
    }
